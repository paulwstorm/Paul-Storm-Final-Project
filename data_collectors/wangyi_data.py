...
response = requests.get("https://c.m.163.com/nc/article/headline/T1348647853363/0-100.html")

headlines = []

for headline in response.json()["T1348647853363"]:
    new_headline = []
    try:
        url = headline["url"]
    except:
    ...
    try:
        image_url = headline["imgsrc"]
    except:
        image_url = "https://upload.wikimedia.org/wikipedia/commons/thumb/6/6b/NetEase_Music_logo.svg/1200px-NetEase_Music_logo.svg.png"  
    try:
        content = headline["title"]
    except:
        content = ""
    try:
        header = headline["digest"]
    except:
        header = ""
    try:
        popularity = "votes " + str(headline["votecount"])
    except:
        popularity = "votes 0"
    date_retrieved = datetime.datetime.now()
    try:
        source = "wangyi - " + headline["source"]
    except:
        source = "wangyi"
    ...
    new_headline.append(header)
    new_headline.append(popularity)
...
headlines = pd.DataFrame(headlines, columns=["url", "image_url", "content", "header", "popularity", "date_retrieved", "source"])
...
for index, row in headlines.iterrows():
    post = Post.objects(postContent = headlines.loc[index]['content']).update(
        ...
        set__postHeader = headlines.loc[index]['header'],
        set__postPopularity = headlines.loc[index]['popularity'],
